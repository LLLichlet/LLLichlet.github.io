...
\begin{proof}
$V^{*} \subseteq F^{V}$, and the pointwise operations are exactly those of the function space $F^{V}$. We use the subspace criteria, where the zero functional $v \mapsto 0$ is linear; the sum of two linear maps is linear; a scalar multiple of a linear map is linear. Hence $V^{*}$ is a subspace of $F^{V}$, and therefore a vector space.
\end{proof}
...
\begin{proof}
The first two follow from the definition of addition and scalar multiplication in $V^{*}$; the last two follow from the linearity of $\varphi$.
\end{proof}
